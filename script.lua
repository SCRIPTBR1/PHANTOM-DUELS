...
-- ============================================================
-- SKY THEME SYSTEM (mantido)
-- ============================================================
local CANDY_SKY_TAG = "PhantomSkyTheme"
...
local MOB_POS_FILE="phantomduels_btnpos.json"
...
local function addShimmerToLabel(lbl,color1,color2)
    ...
    gr.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,color1 or Color3.fromRGB(80,80,80)),ColorSequenceKeypoint.new(0.5,color2 or Color3.fromRGB(200,200,200)),ColorSequenceKeypoint.new(1,color1 or Color3.fromRGB(80,80,80))})
...
local function createRagdollBillboard(duration,labelText,color)
    ...
    local ACCENT = color or Color3.fromRGB(180,180,180)
    ...
    stroke.Color=ACCENT;stroke.Thickness=2;stroke.Transparency=0.05
    task.spawn(function()
        local t=0
    ...
    divider.BackgroundColor3=ACCENT;divider.BackgroundTransparency=0.5;divider.BorderSizePixel=0;divider.ZIndex=31
    ...
    timerLbl.TextColor3=ACCENT;timerLbl.Font=Enum.Font.GothamBlack;timerLbl.TextSize=24
    timerLbl.TextXAlignment=Enum.TextXAlignment.Center;timerLbl.ZIndex=32
    local shimmer=addShimmerToLabel(timerLbl,ACCENT,WHITE)
...
local function setupSpeedIndicator(char)
    ...
    local gr1=addShimmerToLabel(speedLabel,Color3.fromRGB(150,150,150),Color3.fromRGB(255,255,255))
    local gr2=addShimmerToLabel(discordLabel,Color3.fromRGB(120,120,120),Color3.fromRGB(200,200,200))
    ...
    aimbotConn=RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            if not _predBall then
                ...
                _predBall.Color=Color3.fromRGB(100,100,100)
...
refreshSpeedModeLabel=function()
    ...
    if laggerModePillRef and laggerModePillRef.pill and laggerModePillRef.dot then
        ...
        local ON_COLOR=Color3.fromRGB(180,180,180);local OFF=Color3.fromRGB(30,30,30);local WHITE=Color3.fromRGB(255,255,255);local GRAY=Color3.fromRGB(80,80,80)
        TweenService:Create(pill,TweenInfo.new(0.16,Enum.EasingStyle.Quad),{BackgroundColor3=on and ON_COLOR or OFF}):Play()
...
local function createStealBar()
    ...
    local WHITE=Color3.fromRGB(255,255,255)
    local BARBG=Color3.fromRGB(12,12,12)
    ...
    local sbStroke=Instance.new("UIStroke",stealBarFrame);sbStroke.Color=ACCENT;sbStroke.Thickness=1.5;sbStroke.Transparency=0.1
    task.spawn(function()
        local t=0
    ...
    fillLine.BackgroundColor3=ACCENT;fillLine.BorderSizePixel=0;fillLine.ZIndex=21
    Instance.new("UICorner",fillLine).CornerRadius=UDim.new(1,0)
    local fillGrad=Instance.new("UIGradient",fillLine)
    fillGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(100,100,100)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(200,200,200)),ColorSequenceKeypoint.new(1,Color3.fromRGB(100,100,100))})
    ...
    pctLbl.TextColor3=ACCENT;pctLbl.Font=Enum.Font.GothamBlack;pctLbl.TextSize=12
    ...
    div1.BackgroundColor3=ACCENT;div1.BackgroundTransparency=0.6;div1.BorderSizePixel=0;div1.ZIndex=25
...
local function buildMobileButtons()
    ...
    local ACCENT_ON   = Color3.fromRGB(200,200,200)
    local BTN_BG_OFF  = Color3.fromRGB(25,25,25)
    local PANEL_BG    = Color3.fromRGB(15,15,15)
    local BORDER_OFF  = Color3.fromRGB(60,60,60)
    local BORDER_ON   = Color3.fromRGB(200,200,200)
    local TEXT_OFF    = Color3.fromRGB(150,150,150)
    ...
    panelStroke.Color=Color3.fromRGB(50,50,50);panelStroke.Thickness=1.5;panelStroke.Transparency=0.3
    ...
    for i,def in ipairs(buttons) do
        ...
        local function setOn(v)
            ...
            if v then
                ...
                TweenService:Create(frame,TweenInfo.new(0.12,Enum.EasingStyle.Quad),{BackgroundColor3=ACCENT_ON}):Play()
        ...
        local function flash()
            TweenService:Create(frame,TweenInfo.new(0.06),{BackgroundColor3=Color3.fromRGB(100,100,100)}):Play()
...
-- ============================================================
-- MAIN GUI — PHANTOM DUELS (preto/branco/cinza escuro)
-- ============================================================
local function buildGui()
    clearPersistentConns();createStealBar()

    -- Paleta preto/branco/cinza escuro elegante
    local BG         = Color3.fromRGB(8, 8, 8)
    local BG2        = Color3.fromRGB(14, 14, 14)
    local ROW_BG     = Color3.fromRGB(18, 18, 18)
    local ROW_BORDER = Color3.fromRGB(40, 40, 40)
    local WHITE      = Color3.fromRGB(255, 255, 255)
    local GRAY       = Color3.fromRGB(130, 130, 130)
    local GRAY_LIGHT = Color3.fromRGB(180, 180, 180)
    local INP        = Color3.fromRGB(10, 10, 10)
    local ACCENT     = Color3.fromRGB(200, 200, 200)
    local ACCENT_DARK= Color3.fromRGB(60, 60, 60)
    local OFF        = Color3.fromRGB(25, 25, 25)
    local TAB_ACTIVE = Color3.fromRGB(220, 220, 220)
    local TAB_INACT  = Color3.fromRGB(100, 100, 100)
    local SECT_LBL   = Color3.fromRGB(150, 150, 150)
    ...
    local mfStroke=Instance.new("UIStroke",mainFrame);mfStroke.Color=ACCENT;mfStroke.Thickness=1;mfStroke.Transparency=0.3
    task.spawn(function()
        local t=0
        ...
        if not _transpOriginals then
            ...
            local function snapshot(parent)
                for _,child in ipairs(parent:GetChildren()) do
                    if (child:IsA("Frame") or child:IsA("ScrollingFrame")) then _transpOriginals[child]=child.BackgroundTransparency end
                    snapshot(child)
                end
            end
            snapshot(mainFrame)
    ...
    hDivider.BackgroundColor3=ACCENT;hDivider.BackgroundTransparency=0.5;hDivider.BorderSizePixel=0;hDivider.ZIndex=7
    ...
    titleLbl.TextSize=13;titleLbl.TextXAlignment=Enum.TextXAlignment.Left;titleLbl.ZIndex=8
    ...
    discSmall.TextColor3=ACCENT;discSmall.Font=Enum.Font.GothamBold
    ...
    closeBtn.BackgroundColor3=ACCENT_DARK;closeBtn.BorderSizePixel=0
    ...
    miniBtn.Text="PHANTOM DUELS";miniBtn.TextColor3=ACCENT
    miniBtn.Font=Enum.Font.GothamBlack;miniBtn.TextSize=10;miniBtn.ZIndex=20;miniBtn.Visible=false
    ...
    for _,name in ipairs(TABS) do
        ...
        sf.ScrollBarThickness=2;sf.ScrollBarImageColor3=ACCENT_DARK
    ...
    local function animPill(pill, dot, on)
        local ON_COLOR=Color3.fromRGB(180,180,180)
        TweenService:Create(pill,TweenInfo.new(0.15,Enum.EasingStyle.Quad),{BackgroundColor3=on and ON_COLOR or OFF}):Play()
    ...
    local function mkBoxRow(parent, txt, default, cb)
        ...
        tb.Text=tostring(default);tb.TextColor3=ACCENT
    ...
    local function mkKBButton(row, kbEntry, cb)
        ...
        btn.Text=getLabel();btn.TextColor3=ACCENT
    ...
    local function mkActionRow(parent, txt, onActivate)
        ...
        btn.BackgroundColor3=ACCENT_DARK;btn.BorderSizePixel=0
        btn.Text="▶";btn.TextColor3=WHITE;btn.Font=Enum.Font.GothamBlack;btn.TextSize=11
        btn.AutoButtonColor=false;btn.ZIndex=9
        Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
        btn.Activated:Connect(function()
            TweenService:Create(btn,TweenInfo.new(0.06),{BackgroundColor3=Color3.fromRGB(80,80,80)}):Play()
            task.delay(0.12,function() TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=ACCENT_DARK}):Play() end)
    ...
    do
        ...
        manualBtn.BackgroundColor3=(infJumpMode=="manual") and ACCENT or Color3.fromRGB(30,30,30)
        ...
        holdBtn.BackgroundColor3=(infJumpMode=="hold") and ACCENT or Color3.fromRGB(30,30,30)
        ...
        manualBtn.Activated:Connect(function() infJumpMode="manual";manualBtn.BackgroundColor3=ACCENT;holdBtn.BackgroundColor3=Color3.fromRGB(30,30,30);stopHoldInfJump();saveConfig() end)
        holdBtn.Activated:Connect(function() infJumpMode="hold";holdBtn.BackgroundColor3=ACCENT;manualBtn.BackgroundColor3=Color3.fromRGB(30,30,30);if infJumpEnabled then startHoldInfJump() end;saveConfig() end)
    end
    ...
    do
        ...
        skyVal.TextColor3=ACCENT;skyVal.Font=Enum.Font.GothamBold;skyVal.TextSize=9
        ...
        cycleBtn.BackgroundColor3=ACCENT_DARK;cycleBtn.BorderSizePixel=0
    ...
    do
        ...
        fovBtn.Text=tostring(fovValue);fovBtn.TextColor3=ACCENT
    ...
    do
        ...
        resetBtn.BackgroundColor3=Color3.fromRGB(40,15,15);resetBtn.BorderSizePixel=0
    ...
    do
        ...
        sizeBox.Text=tostring(mobileButtonsSize);sizeBox.TextColor3=ACCENT
    ...
    do
        ...
        resetPosBtn.BackgroundColor3=ACCENT_DARK;resetPosBtn.BorderSizePixel=0
