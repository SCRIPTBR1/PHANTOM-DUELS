...
local UI_NAME = "PhantomHub"
local MOBILE_UI_NAME = "PhantomHubMobileButtons"
pcall(function()
...
local function _phantomAutoMobileScale()
...
do
	mobileButtonScaleValue = _phantomAutoMobileScale()
end
...
local showPhantomGui, hidePhantomGui, isPhantomGuiVisible = nil, nil, nil
refreshMobileButtonUi = function()
	...
	for _,data in pairs(mobileButtonFrames) do
		if data.stroke then
			...
			data.stroke.Color=editMobileButtons and Color3.fromRGB(200,200,210) or Color3.fromRGB(40,40,45)
		end
	end
...
local MOVE_KEYS={[Enum.KeyCode.W]=true,[Enum.KeyCode.A]=true,[Enum.KeyCode.S]=true,[Enum.KeyCode.D]=true,
	[Enum.KeyCode.Up]=true,[Enum.KeyCode.Left]=true,[Enum.KeyCode.Right]=true}
...
local function updatePhantomStealBar(dt)
	...
	local targetPct,targetColor,status=0,PHANTOM_COLORS.ACCENT,CONFIG.AUTO_STEAL_ENABLED and "READY" or "IDLE"
	...
	if StealState.active then
		...
		if StealState.phase=="waitingRange" then
			...
			targetColor=PHANTOM_COLORS.ICE
		else
			status="STEALING"
			targetColor=PHANTOM_COLORS.ACCENT
		end
	elseif recent then
		...
		targetColor=success and Color3.fromRGB(180,220,200) or Color3.fromRGB(200,150,160)
...
RunService.RenderStepped:Connect(updatePhantomStealBar)
task.spawn(function()
...
setupSpeedIndicator = function(char,player)
	...
	local old=head:FindFirstChild(player==LP and "PhantomHubSpeedBB" or "PhantomHubOtherSpeedBB");if old then old:Destroy() end
	local bb=Instance.new("BillboardGui",head)
	bb.Name=player==LP and "PhantomHubSpeedBB" or "PhantomHubOtherSpeedBB"
	...
	if player==LP then
		...
		tag.Text=""; tag.TextColor3=Color3.fromRGB(180,180,190)
	...
	val.Text=player==LP and "Speed: 0.0" or "0";val.TextColor3=player==LP and Color3.fromRGB(230,230,235) or PHANTOM_COLORS.ICE
...
local function setupHitCountdownBillboard(char)
	...
	local old = head:FindFirstChild("PhantomHitCountdownBB")
	if old then pcall(function() old:Destroy() end) end
	local bb = Instance.new("BillboardGui",head)
	bb.Name = "PhantomHitCountdownBB"
...
stopHitCountdownSystem = function()
	...
	if char then
		...
		if head then
			local bb = head:FindFirstChild("PhantomHitCountdownBB")
...
showActionNotification = function(text)
	if not actionNotifGui or not actionNotifGui.Parent then
		...
		actionNotifGui.Name = "PhantomActionNotif"
...
task.spawn(function()
	...
	local st = Instance.new("UIStroke", f); st.Color = PHANTOM_COLORS.ACCENT; st.Thickness = 1; st.Transparency = 0.2
	...
	lockTxt.Text = "LOCKED"
	lockTxt.TextColor3 = Color3.fromRGB(230,230,235)
	...
	targetLabel.Text = ""
	targetLabel.TextColor3 = Color3.fromRGB(230,230,235)
...
local PHANTOM_SKY_TAG = "PhantomHubSkyTheme"
_G._PhantomHubSkyMode = _G._PhantomHubSkyMode or "Off"
local phantomOriginalLighting = nil

local PHANTOM_SKY_PRESETS = {
    ["Off"] = {kind = "off"},
    ["Night"] = {
        clock = 22, brightness = 2,
        ambient = {110,110,130}, outAmb = {120,120,140},
        sky = {stars = 4000, moon = 18, sun = 0, moonTex = true},
        atm = {dens = 0.45, color = {120,120,180}, decay = {60,60,100}, glare = 0.5, haze = 1.2},
    },
    ["Aurora"] = {
        clock = 14, brightness = 3,
        ambient = {150,150,150}, outAmb = {160,160,160},
        atm = {dens = 0.55, color = {200,200,255}, decay = {200,200,255}, glare = 2.5, haze = 3},
        clouds = {cover = 0.7, dens = 0.7, color = {240,240,250}},
    },
    ["Sunset"] = {
        clock = 17.2, brightness = 2.5,
        ambient = {170,150,140}, outAmb = {180,160,150},
        sky = {stars = 0, sun = 25, moon = 0},
        atm = {dens = 0.5, color = {255,180,140}, decay = {255,160,130}, glare = 2, haze = 2.5},
        clouds = {cover = 0.55, dens = 0.55, color = {255,220,200}},
    },
    ["Galaxy"] = {
        clock = 0, brightness = 1.5,
        ambient = {70,60,100}, outAmb = {80,70,110},
        sky = {stars = 10000, moon = 30, sun = 0},
        atm = {dens = 0.15, color = {40,40,80}, decay = {20,20,50}, glare = 0.3, haze = 0.5},
    },
}

local function phantomSaveOriginalLighting()
	if phantomOriginalLighting then return end
	phantomOriginalLighting={
	...
	for _,child in ipairs(Lighting:GetChildren()) do
		if child:IsA("Sky") or child:IsA("Atmosphere") then table.insert(phantomOriginalLighting.LightingChildren,child:Clone()) end
	end
	local terrain=workspace:FindFirstChildOfClass("Terrain")
	if terrain then
		for _,child in ipairs(terrain:GetChildren()) do
			if child:IsA("Clouds") then table.insert(phantomOriginalLighting.TerrainChildren,child:Clone()) end
		end
	end
end

local function phantomClearSky(removeAll)
	for _,child in ipairs(Lighting:GetChildren()) do
		if child:GetAttribute(PHANTOM_SKY_TAG) or (removeAll and (child:IsA("Sky") or child:IsA("Atmosphere"))) then pcall(function() child:Destroy() end) end
	end
	local terrain=workspace:FindFirstChildOfClass("Terrain")
	if terrain then
		for _,child in ipairs(terrain:GetChildren()) do
			if child:GetAttribute(PHANTOM_SKY_TAG) or (removeAll and child:IsA("Clouds")) then pcall(function() child:Destroy() end) end
		end
	end
end

local function phantomInstance(className,parent,props)
	local inst=Instance.new(className)
	inst:SetAttribute(PHANTOM_SKY_TAG,true)
	for k,v in pairs(props or {}) do pcall(function() inst[k]=v end) end
	inst.Parent=parent
	return inst
end

local function phantomColor(rgb)
	return Color3.fromRGB(rgb[1],rgb[2],rgb[3])
end

local function PhantomApplyCustomSky(mode)
	phantomSaveOriginalLighting()
	phantomClearSky(true)
	local terrain=workspace:FindFirstChildOfClass("Terrain")
	local preset=PHANTOM_SKY_PRESETS[mode]
	if not preset or preset.kind=="off" then
		if phantomOriginalLighting then
			for k,v in pairs(phantomOriginalLighting) do
				if k~="LightingChildren" and k~="TerrainChildren" then pcall(function() Lighting[k]=v end) end
			end
			for _,child in ipairs(phantomOriginalLighting.LightingChildren or {}) do child:Clone().Parent=Lighting end
			local offTerrain=workspace:FindFirstChildOfClass("Terrain")
			if offTerrain then
				for _,child in ipairs(phantomOriginalLighting.TerrainChildren or {}) do child:Clone().Parent=offTerrain end
			end
		end
		_G._PhantomHubSkyMode="Off"
		return
	end
	...
	if preset.outAmb then Lighting.OutdoorAmbient=phantomColor(preset.outAmb) end
	if preset.ambient then Lighting.Ambient=phantomColor(preset.ambient) end
	...
	if preset.sky then
		...
		phantomInstance("Sky",Lighting,skyProps)
	end

	if preset.atm then
		phantomInstance("Atmosphere",Lighting,{
			Density=preset.atm.dens or 0.3,
			Color=phantomColor(preset.atm.color),
			Decay=phantomColor(preset.atm.decay),
	...
	if preset.clouds and terrain then
		phantomInstance("Clouds",terrain,{
			Cover=preset.clouds.cover or 0.5,
			Density=preset.clouds.dens or 0.5,
			Color=phantomColor(preset.clouds.color)
		})
	end

	_G._PhantomHubSkyMode=mode
end

local PhantomSkyOrder={{"Off","Off"},{"Night","Night"},{"Aurora","Aurora"},{"Sunset","Sunset"},{"Galaxy","Galaxy"}}
...
local function saveConfig()
	...
	local cfg={
		...
		mainPanelOpen=(_G._phantomMainOpen~=false),
		mainPanelPos=_G._phantomMainPanelPos,
		showFpsPing=(_G._phantomShowFpsPing~=false),
		skyTheme=currentSkyTheme
	}
	if writefile then pcall(function() writefile("PhantomHub.json",HS:JSONEncode(cfg)) end) end
end
...
local function playPhantomIntro(gui,main,miniBtn)
	...
	logo.Size=UDim2.new(0,58,0,58);logo.Position=UDim2.new(0.5,-29,0.5,-108);logo.BackgroundColor3=PHANTOM_COLORS.CARD;logo.BackgroundTransparency=1;logo.BorderSizePixel=0;logo.ZIndex=201
	Instance.new("UICorner",logo).CornerRadius=UDim.new(0,14)
	local logoStroke=Instance.new("UIStroke",logo);logoStroke.Color=PHANTOM_COLORS.ACCENT;logoStroke.Thickness=1.2;logoStroke.Transparency=0.1
	local mark=Instance.new("TextLabel",logo)
	mark.Size=UDim2.new(1,0,1,0);mark.BackgroundTransparency=1;mark.Text="P";mark.TextColor3=PHANTOM_COLORS.ACCENT;mark.Font=Enum.Font.GothamBlack;mark.TextSize=34;mark.TextTransparency=1;mark.ZIndex=202
	local title=Instance.new("TextLabel",intro)
	title.Size=UDim2.new(0,520,0,52);title.Position=UDim2.new(0.5,-260,0.5,-36)
	title.BackgroundTransparency=1;title.Text=PHANTOM_BRAND;title.TextColor3=PHANTOM_COLORS.TEXT
	...
	sub.BackgroundTransparency=1;sub.Text="LOADING ASSETS...";sub.TextColor3=PHANTOM_COLORS.ICE
	...
	loading.Size=UDim2.new(0,520,0,18);loading.Position=UDim2.new(0.5,-260,0.5,72);loading.BackgroundTransparency=1;loading.Text="";loading.TextColor3=PHANTOM_COLORS.SECONDARY;loading.Font=Enum.Font.GothamBold;loading.TextSize=11;loading.TextTransparency=1;loading.TextXAlignment=Enum.TextXAlignment.Center;loading.ZIndex=201
	...
	bar.Size=UDim2.new(0,0,1,0);bar.BackgroundColor3=PHANTOM_COLORS.TEXT;bar.BorderSizePixel=0;bar.ZIndex=202
	...
	local function finishIntro()
		...
		blink=false
		sub.Text="READY"
		sub.TextColor3=PHANTOM_COLORS.ACCENT
		task.wait(0.15)
	...
	skipHint.Text="tap anywhere to skip";skipHint.TextColor3=PHANTOM_COLORS.SECONDARY;skipHint.Font=Enum.Font.GothamMedium;skipHint.TextSize=10;skipHint.TextTransparency=1;skipHint.TextXAlignment=Enum.TextXAlignment.Center;skipHint.ZIndex=204
...
local function buildGui()
	local BG=PHANTOM_COLORS.BG
	local PANEL=PHANTOM_COLORS.PANEL
	local CARD=PHANTOM_COLORS.CARD
	local ACCENT=PHANTOM_COLORS.ACCENT
	local ICE=PHANTOM_COLORS.ICE
	local HOVER=PHANTOM_COLORS.HOVER
	local W=PHANTOM_COLORS.TEXT
	local DIM=PHANTOM_COLORS.SECONDARY
	local STROKE=PHANTOM_COLORS.STROKE
	local INP=PHANTOM_COLORS.INPUT
	local OFF=PHANTOM_COLORS.OFF
	...
	local old=game:GetService("CoreGui"):FindFirstChild("PhantomHub");if old then old:Destroy() end
	local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("PhantomHub");if o then o:Destroy() end end
	local gui=Instance.new("ScreenGui")
	gui.Name="PhantomHub";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
	...
	local function buildInstaResetPanel()
		...
		panel.Name="PhantomInstaResetPanel"
		...
		title.Size=UDim2.new(1,-78,0,28);title.Position=UDim2.new(0,22,0,18);title.BackgroundTransparency=1;title.Text="PHANTOM INSTA RESET";title.TextColor3=Color3.fromRGB(230,230,235);title.Font=Enum.Font.GothamBlack;title.TextSize=17;title.TextXAlignment=Enum.TextXAlignment.Left;title.ZIndex=62
		...
		reset.Text=resetKeyText();reset.TextColor3=Color3.fromRGB(0,0,0);reset.Font=Enum.Font.GothamBlack;reset.TextSize=18;reset.AutoButtonColor=false;reset.ZIndex=62
		Instance.new("UICorner",reset).CornerRadius=UDim.new(0,12)
		local resetStroke=Instance.new("UIStroke",reset);resetStroke.Color=Color3.fromRGB(200,200,210);resetStroke.Thickness=1;resetStroke.Transparency=0.4
		...
		reset.Activated:Connect(function() 
			-- Only show notification, no actual reset
			if showActionNotification then showActionNotification("RESET!") end
		end)
	...
	drag(main,function(active,moved)
		if not active and moved then
			_G._phantomMainPanelPos={xs=main.Position.X.Scale,x=main.Position.X.Offset,ys=main.Position.Y.Scale,y=main.Position.Y.Offset}
			pcall(saveConfig)
		end
	end)
	...
	ttl.BackgroundTransparency=1;ttl.Text=PHANTOM_BRAND;ttl.TextColor3=Color3.fromRGB(230,230,235);ttl.Font=Enum.Font.GothamBlack;ttl.TextSize=IsMobile and 14 or 17;ttl.TextXAlignment=Enum.TextXAlignment.Left
	...
	sub.BackgroundTransparency=1;sub.Text="";sub.TextColor3=Color3.fromRGB(180,180,190);sub.Font=Enum.Font.GothamMedium;sub.TextSize=IsMobile and 9 or 10;sub.TextXAlignment=Enum.TextXAlignment.Left
	...
	lockBtn.Size=UDim2.new(0,IsMobile and 34 or 42,0,IsMobile and 23 or 26);lockBtn.Position=UDim2.new(1,IsMobile or -70 or -88,0,IsMobile and 11 or 14);lockBtn.BackgroundColor3=Color3.fromRGB(200,200,210);lockBtn.BorderSizePixel=0
	lockBtn.Text=uiLocked and "🔒" or "🔓";lockBtn.TextColor3=Color3.fromRGB(0,0,0);lockBtn.Font=Enum.Font.GothamBlack;lockBtn.TextSize=IsMobile and 13 or 15;lockBtn.AutoButtonColor=false
	Instance.new("UICorner",lockBtn).CornerRadius=UDim.new(1,0)
	local lockStroke=Instance.new("UIStroke",lockBtn);lockStroke.Color=ACCENT;lockStroke.Thickness=1;lockStroke.Transparency=0.34
	...
	miniBtn.Text="Phantom Hub";miniBtn.TextColor3=W;miniBtn.Font=Enum.Font.GothamBold;miniBtn.TextSize=12;miniBtn.ZIndex=20;miniBtn.Visible=false
	...
	local function showGui() main.Visible=true;miniBtn.Visible=false;_G._phantomMainOpen=true;pcall(saveConfig) end
	local function hideGui()
		...
		_G._phantomMainOpen=false;pcall(saveConfig)
	end
	showPhantomGui=showGui;hidePhantomGui=hideGui;isPhantomGuiVisible=function() return main.Visible end
	...
	_G._phantomFpsLbl=fpsLbl
	_G._phantomPingLbl=pingLbl
	_G._phantomSepFps=sepFps
	_G._phantomSepPing=sepPing
	local showFpsPing = _G._phantomShowFpsPing
	...
	progressRadLbl.Text=tostring(CONFIG.STEAL_RANGE);progressRadLbl.TextColor3=Color3.fromRGB(230,230,235);progressRadLbl.Font=Enum.Font.GothamBlack;progressRadLbl.TextSize=11;progressRadLbl.TextXAlignment=Enum.TextXAlignment.Center
	...
	RunService.RenderStepped:Connect(function(dt)
		...
		if isIdle then
			...
			progressFill.BackgroundColor3=progressFill.BackgroundColor3:Lerp(PHANTOM_COLORS.ACCENT,math.min(dt*8,1))
			...
			if idleDotTimer>=0.35 then
				...
				progressPct.TextColor3=PHANTOM_COLORS.ACCENT
			end
		end
	end)
	...
	local function mkRow(parent,h,toggleRow)
		...
		local hover=toggleRow and Color3.fromRGB(20,20,28) or Color3.fromRGB(16,16,20)
	...
	local function mkPill(row,offset)
		...
		grad.Size=UDim2.new(1,0,1,0);grad.Position=UDim2.new(0,0,0,0);grad.BackgroundColor3=Color3.fromRGB(200,200,210);grad.BackgroundTransparency=1;grad.BorderSizePixel=0;grad.ZIndex=3
	...
	local function animPill(pill,dot,on,grad,pillStroke)
		...
		TS:Create(dot,TweenInfo.new(0.28,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
			...
			BackgroundColor3=on and Color3.fromRGB(0,0,0) or Color3.fromRGB(180,180,200),
	...
	local function mkKB(parent,kbEntry,cb)
		local btn=Instance.new("TextButton",parent);btn.Size=UDim2.new(0,58,0,22);btn.Position=UDim2.new(1,-66,0.5,-11);btn.BackgroundColor3=Color3.fromRGB(200,200,210);btn.BorderSizePixel=0
		local function getLabel() return (kbEntry.gp and kbEntry.gp.Name) or (kbEntry.kb and kbEntry.kb.Name) or "None" end
		btn.Text=getLabel();btn.TextColor3=Color3.fromRGB(0,0,0);btn.Font=Enum.Font.GothamBlack;btn.TextSize=12;btn.ZIndex=5
	...
	local function mkSizeSelector(parent,txt,getValue,setValue,setters)
		...
		for i,v in ipairs(SCALE_OPTIONS) do
			...
			btn.BackgroundColor3=Color3.fromRGB(200,200,210);btn.BorderSizePixel=0;btn.Text=tostring(math.floor(v*100)).."%";btn.TextColor3=Color3.fromRGB(0,0,0);btn.Font=Enum.Font.GothamBlack;btn.TextSize=IsMobile and 8 or 9;btn.AutoButtonColor=false;btn.ZIndex=4
			Instance.new("UICorner",btn).CornerRadius=UDim.new(1,0)
		...
		local function refresh()
			for _,item in ipairs(buttons) do
				...
				item.btn.TextColor3=active and Color3.fromRGB(0,0,0) or Color3.fromRGB(180,180,200)
	...
	do
		...
		_G._phantomAimbotValBox = aimbotBox
	end
	...
	do
		...
		local skyIndex=1
		local current=tostring(currentSkyTheme or _G._PhantomHubSkyMode or PhantomSkyOrder[1][2])
		for i,entry in ipairs(PhantomSkyOrder) do if entry[2]==current then skyIndex=i;break end end
		local skyVal=Instance.new("TextLabel",row);skyVal.Size=UDim2.new(0,150,1,0);skyVal.Position=UDim2.new(1,-158,0,0);skyVal.BackgroundTransparency=1;skyVal.Text=PhantomSkyOrder[skyIndex][2];skyVal.TextColor3=ACCENT;skyVal.Font=Enum.Font.GothamBlack;skyVal.TextSize=11;skyVal.TextXAlignment=Enum.TextXAlignment.Right
		local clk=Instance.new("TextButton",row);clk.Size=UDim2.new(1,0,1,0);clk.BackgroundTransparency=1;clk.Text="";clk.ZIndex=2
		clk.Activated:Connect(function()
			if _anyKeyListening then return end
			skyIndex=skyIndex%#PhantomSkyOrder+1
			local label=PhantomSkyOrder[skyIndex][2]
			skyVal.Text=label
			currentSkyTheme=label
			PhantomApplyCustomSky(label)
			saveConfig()
		end)
	end
	...
	setShowFpsPingVisual=mkToggle(mainPage,"Show FPS/Ping",function(on)
		_G._phantomShowFpsPing=on
		if _G._phantomFpsLbl then _G._phantomFpsLbl.Visible=on end
		if _G._phantomPingLbl then _G._phantomPingLbl.Visible=on end
		if _G._phantomSepFps then _G._phantomSepFps.Visible=on end
		if _G._phantomSepPing then _G._phantomSepPing.Visible=on end
		saveConfig()
	end)
	...
	do local row=mkRow(mainPage,ROW_H);mkLabel(row,"Reset Mobile Buttons");local btn=Instance.new("TextButton",row);btn.Size=UDim2.new(0,86,0,22);btn.Position=UDim2.new(1,-94,0.5,-11);btn.BackgroundColor3=Color3.fromRGB(200,200,210);btn.BorderSizePixel=0;btn.Text="RESET";btn.TextColor3=Color3.fromRGB(0,0,0);btn.Font=Enum.Font.GothamBlack;btn.TextSize=12;btn.AutoButtonColor=false;btn.ZIndex=3;Instance.new("UICorner",btn).CornerRadius=UDim.new(1,0);local rst=Instance.new("UIStroke",btn);rst.Color=ACCENT;rst.Thickness=1;rst.Transparency=0.34;btn.MouseEnter:Connect(function() TS:Create(rst,TweenInfo.new(0.12),{Transparency=0.02}):Play() end);btn.MouseLeave:Connect(function() TS:Create(rst,TweenInfo.new(0.12),{Transparency=0.34}):Play() end);btn.Activated:Connect(function() if _anyKeyListening then return end;resetMobileButtonLayout();saveConfig() end) end
	...
	do local row=mkRow(mainPage,ROW_H);mkLabel(row,"Insta Reset");local btn=Instance.new("TextButton",row);btn.Size=UDim2.new(0,86,0,22);btn.Position=UDim2.new(1,-94,0.5,-11);btn.BackgroundColor3=Color3.fromRGB(200,200,210);btn.BorderSizePixel=0;btn.Text="OPEN";btn.TextColor3=Color3.fromRGB(0,0,0);btn.Font=Enum.Font.GothamBlack;btn.TextSize=12;btn.AutoButtonColor=false;btn.ZIndex=3;Instance.new("UICorner",btn).CornerRadius=UDim.new(1,0);local opn=Instance.new("UIStroke",btn);opn.Color=ACCENT;opn.Thickness=1;opn.Transparency=0.34;btn.MouseEnter:Connect(function() TS:Create(opn,TweenInfo.new(0.12),{Transparency=0.02}):Play() end);btn.MouseLeave:Connect(function() TS:Create(opn,TweenInfo.new(0.12),{Transparency=0.34}):Play() end);btn.Activated:Connect(function() if _anyKeyListening then return end;if setInstaResetPanelVisible then setInstaResetPanelVisible(true) end end) end
	...
	if _G._phantomMainPanelPos then
		local p=_G._phantomMainPanelPos
		main.Position=UDim2.new(p.xs or 0.5,p.x or -180,p.ys or 0.5,p.y or -150)
	end
	if _G._phantomMainOpen==false then
	...
	playPhantomIntro(gui,main,miniBtn)
end
...
local function buildMobileButtons()
	...
	editBanner.BackgroundColor3=Color3.fromRGB(200,200,210)
	...
	ebStroke.Color=Color3.fromRGB(200,200,210);ebStroke.Thickness=1;ebStroke.Transparency=0.4
	local ebLbl=Instance.new("TextLabel",editBanner)
	ebLbl.Size=UDim2.new(1,0,1,0);ebLbl.BackgroundTransparency=1
	ebLbl.Text="EDIT MODE";ebLbl.TextColor3=Color3.fromRGB(0,0,0);ebLbl.Font=Enum.Font.GothamBlack;ebLbl.TextSize=12
	ebLbl.ZIndex=11
	...
	local function makeBtn(id,label,col,row,defaultLabel)
		...
		f.BackgroundColor3=PHANTOM_COLORS.CARD
		f.BorderSizePixel=0
		...
		gradLayer.BackgroundColor3=Color3.fromRGB(200,200,210)
		gradLayer.BackgroundTransparency=1
		gradLayer.BorderSizePixel=0
		gradLayer.ZIndex=1
		Instance.new("UICorner",gradLayer).CornerRadius=UDim.new(0,9)

		local st=Instance.new("UIStroke",f);st.Color=PHANTOM_COLORS.STROKE;st.Thickness=1;st.Transparency=0.34
		...
		flashLayer.BackgroundColor3=Color3.fromRGB(200,200,210)
		...
		lbl.TextColor3=PHANTOM_COLORS.TEXT
		lbl.Font=Enum.Font.GothamBlack
		lbl.TextSize=9
		...
		mobileButtonFrames[id]={frame=f,defaultPosition=defPos,stroke=st,label=lbl,button=btn,gradLayer=gradLayer,flashLayer=flashLayer}
	...
	RunService.Heartbeat:Connect(function(dt)
		...
		local function setActive(id,active)
			...
			data.label.TextColor3 = PHANTOM_COLORS.TEXT:Lerp(Color3.fromRGB(0,0,0), intensity)
...
local function loadConfigKeys()
	...
	local raw=nil
	pcall(function() if isfile("PhantomHub.json") then raw=readfile("PhantomHub.json") end end)
	...
	if cfg.mainPanelOpen~=nil then _G._phantomMainOpen=cfg.mainPanelOpen end
	if type(cfg.mainPanelPos)=="table" then _G._phantomMainPanelPos=cfg.mainPanelPos end
	if cfg.showFpsPing~=nil then
		_G._phantomShowFpsPing=cfg.showFpsPing
		if setShowFpsPingVisual then setShowFpsPingVisual(cfg.showFpsPing) end
		if _G._phantomFpsLbl then _G._phantomFpsLbl.Visible=cfg.showFpsPing end
		if _G._phantomPingLbl then _G._phantomPingLbl.Visible=cfg.showFpsPing end
		if _G._phantomSepFps then _G._phantomSepFps.Visible=cfg.showFpsPing end
		if _G._phantomSepPing then _G._phantomSepPing.Visible=cfg.showFpsPing end
	end
	...
	_G._PhantomHubLoadedState=cfg
end

local function loadConfigState()
	local cfg=_G._PhantomHubLoadedState
	...
	if currentSkyTheme and currentSkyTheme~="Off" then PhantomApplyCustomSky(currentSkyTheme) end
	...
	if _G._phantomAimbotValBox then _G._phantomAimbotValBox.Text=tostring(aimbotSpeed) end
