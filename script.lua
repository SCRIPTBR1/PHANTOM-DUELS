...
-- ------------------------------------------------------------
-- INTRO SOUND (only if enabled) – now uses new filename and always redownloads
-- ------------------------------------------------------------
...
local function createRagdollBillboard(duration,labelText,color)
    ...
    local ACCENT  = color or Color3.fromRGB(200,200,200)
    ...
    -- Thick animated border
    local stroke=Instance.new("UIStroke",card)
    stroke.Color=ACCENT;stroke.Thickness=3;stroke.Transparency=0.05
    task.spawn(function()
        local t=0
        while stroke and stroke.Parent do
            t=t+0.05
            stroke.Transparency=0.02+math.abs(math.sin(t*2.5))*0.25
            local gray=math.clamp(150+math.abs(math.sin(t*1.5))*50,100,255)
...
local function onHumanoidStateChanged(old,new)
    ...
    if isRag and not hum.PlatformStand and not activeBatBillboard then
        activeBatBillboard=createRagdollBillboard(2.6,"RAGDOLL",Color3.fromRGB(200,200,200))
...
refreshSpeedModeLabel=function()
    ...
    if laggerModePillRef and laggerModePillRef.pill and laggerModePillRef.dot then
        ...
        local ON=Color3.fromRGB(180,180,180);local OFF=Color3.fromRGB(30,30,30);local WHITE=Color3.fromRGB(255,255,255);local GRAY=Color3.fromRGB(120,120,120)
        TweenService:Create(pill,TweenInfo.new(0.16,Enum.EasingStyle.Quad),{BackgroundColor3=on and ON or OFF}):Play()
...
local function createStealBar()
    ...
    local WHITE=Color3.fromRGB(255,255,255)
    local BARBG=Color3.fromRGB(15,15,15)
    ...
    local sbStroke=Instance.new("UIStroke",stealBarFrame);sbStroke.Color=ACCENT;sbStroke.Thickness=2;sbStroke.Transparency=0.1
    task.spawn(function()
        local t=0
        while sbStroke and sbStroke.Parent do
            t=t+0.05;sbStroke.Transparency=0.05+math.abs(math.sin(t*2))*0.35
            local gray=math.floor(150+math.abs(math.sin(t*1.5))*70)
            sbStroke.Color=Color3.fromRGB(gray,gray,gray)
            task.wait(0.04)
        end
    end)
    ...
    fillGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(150,150,150)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(200,200,200)),ColorSequenceKeypoint.new(1,Color3.fromRGB(150,150,150))})
...
local function buildMobileButtons()
    ...
    local BTN_BG_OFF    = Color3.fromRGB(30,30,30)
    local PANEL_BG      = Color3.fromRGB(15,15,15)
    local BORDER_OFF    = Color3.fromRGB(80,80,80)
    ...
    panelStroke.Color=Color3.fromRGB(60,60,60);panelStroke.Thickness=2;panelStroke.Transparency=0.3
    ...
    for i,def in ipairs(buttons) do
        ...
        local function setOn(v)
            ...
            if v then
                ...
                TweenService:Create(frame,TweenInfo.new(0.12,Enum.EasingStyle.Quad),{BackgroundColor3=ON_COLOR}):Play()
        ...
        local function flash()
            TweenService:Create(frame,TweenInfo.new(0.06),{BackgroundColor3=Color3.fromRGB(200,200,200)}):Play()
...
-- ============================================================
-- MAIN GUI — 🌹PHANTOM DUELS (Black, White & Dark Gray theme)
-- ============================================================
local function buildGui()
    ...
    local INP        = Color3.fromRGB(8, 8, 8)
    local ACCENT     = Color3.fromRGB(180, 180, 180)
    local ACCENT2    = Color3.fromRGB(160, 160, 160)
    local ACCENT3    = Color3.fromRGB(70, 70, 70)
    local OFF        = Color3.fromRGB(30, 30, 30)
    local TAB_ACTIVE = Color3.fromRGB(200, 200, 200)
    local TAB_INACT  = Color3.fromRGB(100, 100, 100)
    local SECT_LBL   = Color3.fromRGB(160, 160, 160)
    ...
    local mfStroke=Instance.new("UIStroke",mainFrame);mfStroke.Color=ACCENT;mfStroke.Thickness=1.5;mfStroke.Transparency=0.3
    task.spawn(function()
        local t=0
        while mfStroke and mfStroke.Parent do
            t=t+0.05
            mfStroke.Transparency=0.4+math.sin(t*3)*0.25
            local gray=math.floor(150+math.sin(t*2)*35)
            mfStroke.Color=Color3.fromRGB(gray,gray,gray)
            task.wait(0.04)
        end
    end)
    ...
    hDivider.BackgroundColor3=ACCENT;hDivider.BackgroundTransparency=0.6;hDivider.BorderSizePixel=0;hDivider.ZIndex=7

    dragHandle(header)

    -- Title with rose emoji
    ...
    discSmall.Size=UDim2.new(0,140,0,12);discSmall.Position=UDim2.new(0,12,1,-14)
    ...
    closeBtn.BackgroundColor3=ACCENT3;closeBtn.BorderSizePixel=0
    ...
    miniBtn.Size=UDim2.new(0,130,0,28);miniBtn.Position=UDim2.new(0,26,0,26)
    ...
    for _,name in ipairs(TABS) do
        ...
        sf.ScrollBarThickness=2;sf.ScrollBarImageColor3=ACCENT3
    ...
    local function animPill(pill, dot, on)
        ...
        TweenService:Create(pill,TweenInfo.new(0.15,Enum.EasingStyle.Quad),{BackgroundColor3=on and ON_COLOR or OFF_COLOR}):Play()
    ...
    local function mkActionRow(parent, txt, onActivate)
        ...
        btn.BackgroundColor3=ACCENT3;btn.BorderSizePixel=0
        btn.Text="▶";btn.TextColor3=WHITE;btn.Font=Enum.Font.GothamBlack;btn.TextSize=11
        btn.AutoButtonColor=false;btn.ZIndex=9
        Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
        btn.Activated:Connect(function()
            TweenService:Create(btn,TweenInfo.new(0.06),{BackgroundColor3=Color3.fromRGB(100,100,100)}):Play()
            task.delay(0.12,function() TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=ACCENT3}):Play() end)
    ...
    do
        ...
        manualBtn.BackgroundColor3=(infJumpMode=="manual") and ACCENT2 or Color3.fromRGB(40,40,40)
        ...
        holdBtn.BackgroundColor3=(infJumpMode=="hold") and ACCENT2 or Color3.fromRGB(40,40,40)
        ...
        manualBtn.Activated:Connect(function() infJumpMode="manual";manualBtn.BackgroundColor3=ACCENT2;holdBtn.BackgroundColor3=Color3.fromRGB(40,40,40);stopHoldInfJump();saveConfig() end)
        holdBtn.Activated:Connect(function() infJumpMode="hold";holdBtn.BackgroundColor3=ACCENT2;manualBtn.BackgroundColor3=Color3.fromRGB(40,40,40);if infJumpEnabled then startHoldInfJump() end;saveConfig() end)
    end
    ...
    do
        ...
        cycleBtn.BackgroundColor3=ACCENT3;cycleBtn.BorderSizePixel=0
    ...
    do
        ...
        resetBtn.BackgroundColor3=Color3.fromRGB(60,25,25);resetBtn.BorderSizePixel=0
    ...
    do
        ...
        resetPosBtn.BackgroundColor3=ACCENT3;resetPosBtn.BorderSizePixel=0
