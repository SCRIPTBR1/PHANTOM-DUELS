...
if isfile and isfile("phantomduels.json") then
    local ok, data = pcall(function() return HS:JSONDecode(readfile("phantomduels.json")) end)
...
if introSoundEnabled then
    ...
    local numeFisier = "phantom_intro.mp3"
...
-- ============================================================
-- SKY THEME SYSTEM - Monochrome themes
-- ============================================================
local PHANTOM_SKY_TAG = "PhantomSkyTheme"
local currentSkyTheme = "Night"
local PHANTOM_SKY_PRESETS = {
    ["Off"]={kind="off"},
    ["Night"]={clock=22,brightness=2,ambient={60,60,65},outAmb={70,70,75},sky={stars=4000,moon=18,sun=0,moonTex=true},atm={dens=0.45,color={80,80,90},decay={50,50,60},glare=0.5,haze=1.2}},
    ["Dark"]={clock=0,brightness=1.5,ambient={30,30,35},outAmb={40,40,45},sky={stars=6000,moon=24,sun=0,moonTex=true},atm={dens=0.5,color={50,50,60},decay={30,30,40},glare=0.6,haze=1.5}},
    ["Gray Dawn"]={clock=6.5,brightness=2.8,ambient={160,160,165},outAmb={170,170,175},sky={sun=18,moon=0,stars=0},atm={dens=0.4,color={180,180,190},decay={140,140,150},glare=1.8,haze=2.2}},
    ["Silver"]={clock=14,brightness=3.5,ambient={200,200,205},outAmb={210,210,215},atm={dens=0.3,color={200,200,210},decay={180,180,190},glare=1.5,haze=1.5},clouds={cover=0.5,dens=0.5,color={220,220,230}}},
    ["Storm"]={clock=15,brightness=1.4,ambient={70,70,80},outAmb={80,80,90},sky={stars=0,sun=6,moon=0},atm={dens=0.65,color={80,80,100},decay={50,50,70},glare=0.5,haze=3},clouds={cover=0.95,dens=0.95,color={60,60,70}}},
}
local SkyOrder={"Off","Night","Dark","Gray Dawn","Silver","Storm"}

local function phantomColor(rgb) return Color3.fromRGB(rgb[1],rgb[2],rgb[3]) end

local function PhantomApplyCustomSky(mode)
    for _,child in ipairs(Lighting:GetChildren()) do if child:GetAttribute(PHANTOM_SKY_TAG) then pcall(function() child:Destroy() end) end end
    local terrain=workspace:FindFirstChildOfClass("Terrain")
    if terrain then for _,child in ipairs(terrain:GetChildren()) do if child:GetAttribute(PHANTOM_SKY_TAG) then pcall(function() child:Destroy() end) end end end
    ...
    Lighting.FogStart=0;Lighting.FogEnd=100000;Lighting.FogColor=Color3.fromRGB(180,180,180);Lighting.ColorShift_Top=Color3.fromRGB(0,0,0);Lighting.ColorShift_Bottom=Color3.fromRGB(0,0,0);Lighting.GlobalShadows=true
    ...
    if preset.sky then
        local skyInst=Instance.new("Sky");skyInst:SetAttribute(PHANTOM_SKY_TAG,true)
    ...
    if preset.atm then
        local atm=Instance.new("Atmosphere");atm:SetAttribute(PHANTOM_SKY_TAG,true)
        atm.Density=preset.atm.dens or 0.3;atm.Color=phantomColor(preset.atm.color);atm.Decay=phantomColor(preset.atm.decay);atm.Glare=preset.atm.glare or 1;atm.Haze=preset.atm.haze or 1;atm.Parent=Lighting
    end
    if preset.clouds and terrain then
        local clouds=Instance.new("Clouds");clouds:SetAttribute(PHANTOM_SKY_TAG,true)
        clouds.Cover=preset.clouds.cover or 0.5;clouds.Density=preset.clouds.dens or 0.5;clouds.Color=phantomColor(preset.clouds.color);clouds.Parent=terrain
    end
end
...
local MOB_POS_FILE="phantom_btnpos.json"
...
local function addShimmerToLabel(lbl,color1,color2)
    ...
    gr.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,color1 or Color3.fromRGB(180,180,180)),ColorSequenceKeypoint.new(0.5,color2 or Color3.fromRGB(255,255,255)),ColorSequenceKeypoint.new(1,color1 or Color3.fromRGB(180,180,180))})
...
local function createRagdollBillboard(duration,labelText,color)
    ...
    local GRAY   = color or Color3.fromRGB(180,180,180)
    local BG     = Color3.fromRGB(15,15,18)
    local WHITE  = Color3.fromRGB(255,255,255)
    local W,H    = 210,80
    local guiName="PhantomRagdollTimer_"..labelText
    pcall(function()
    ...
    stroke.Color=GRAY;stroke.Thickness=3;stroke.Transparency=0.05
    task.spawn(function()
        local t=0
    ...
    divider.BackgroundColor3=GRAY;divider.BackgroundTransparency=0.5;divider.BorderSizePixel=0;divider.ZIndex=31
    ...
    timerLbl.TextColor3=GRAY;timerLbl.Font=Enum.Font.GothamBlack;timerLbl.TextSize=24
    timerLbl.TextXAlignment=Enum.TextXAlignment.Center;timerLbl.ZIndex=32
    local shimmer=addShimmerToLabel(timerLbl,GRAY,WHITE)
...
local function onHumanoidStateChanged(old,new)
    ...
    if isRag and not hum.PlatformStand and not activeBatBillboard then
        activeBatBillboard=createRagdollBillboard(2.6,"RAGDOLL",Color3.fromRGB(180,180,180))
...
local function onMedusaStateChanged()
    ...
    if hum and hum.PlatformStand and not activeMedusaBillboard then
        activeMedusaBillboard=createRagdollBillboard(4.5,"STONE",Color3.fromRGB(200,200,200))
...
local function setupSpeedIndicator(char)
    ...
    if head:FindFirstChild("PhantomSpeedBB") then head.PhantomSpeedBB:Destroy() end
    local bb=Instance.new("BillboardGui",head);bb.Name="PhantomSpeedBB";bb.Size=UDim2.new(0,140,0,52);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true
    local discordLabel=Instance.new("TextLabel",bb);discordLabel.Size=UDim2.new(1,0,0.4,0);discordLabel.BackgroundTransparency=1;discordLabel.Text="PHANTOM DUELS"
    discordLabel.TextColor3=Color3.fromRGB(180,180,180);discordLabel.Font=Enum.Font.GothamBold;discordLabel.TextScaled=true;discordLabel.TextStrokeTransparency=0
    ...
    local gr1=addShimmerToLabel(speedLabel,Color3.fromRGB(180,180,180),Color3.fromRGB(255,255,255))
    local gr2=addShimmerToLabel(discordLabel,Color3.fromRGB(150,150,150),Color3.fromRGB(255,255,255))
...
local STRETCH_NAME="Phantom_Stretch"
    ...
    aimbotConn=RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            if not _predBall then
                ...
                _predBall.Color=Color3.fromRGB(180,180,180)
...
saveConfig=function()
    ...
    if writefile then pcall(function() writefile("phantomduels.json",HS:JSONEncode(cfg)) end) end
end
...
refreshSpeedModeLabel=function()
    ...
    if laggerModePillRef and laggerModePillRef.pill and laggerModePillRef.dot then
        ...
        local GRAY_ON=Color3.fromRGB(180,180,180);local OFF=Color3.fromRGB(30,30,35);local WHITE=Color3.fromRGB(255,255,255);local GRAY_OFF=Color3.fromRGB(100,100,100)
        TweenService:Create(pill,TweenInfo.new(0.16,Enum.EasingStyle.Quad),{BackgroundColor3=on and GRAY_ON or OFF}):Play()
        TweenService:Create(dot,TweenInfo.new(0.16,Enum.EasingStyle.Back),{Position=on and UDim2.new(1,-13,0.5,-5) or UDim2.new(0,3,0.5,-5),BackgroundColor3=on and WHITE or GRAY_OFF}):Play()
    end
...
local function createStealBar()
    for _,n in ipairs({"PhantomStealBar"}) do
    ...
    local WHITE=Color3.fromRGB(255,255,255)
    local BARBG=Color3.fromRGB(15,15,18)
    local SB_W,SB_H=260,32
    local stealGui=Instance.new("ScreenGui");stealGui.Name="PhantomStealBar";stealGui.ResetOnSpawn=false;stealGui.IgnoreGuiInset=true;stealGui.DisplayOrder=8
    ...
    local sbStroke=Instance.new("UIStroke",stealBarFrame);sbStroke.Color=GRAY;sbStroke.Thickness=2;sbStroke.Transparency=0.1
    task.spawn(function()
        local t=0
    ...
    fillLine.BackgroundColor3=GRAY;fillLine.BorderSizePixel=0;fillLine.ZIndex=21
    Instance.new("UICorner",fillLine).CornerRadius=UDim.new(1,0)
    local fillGrad=Instance.new("UIGradient",fillLine)
    fillGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(160,160,160)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(200,200,200)),ColorSequenceKeypoint.new(1,Color3.fromRGB(160,160,160))})
    ...
    pctLbl.TextColor3=GRAY;pctLbl.Font=Enum.Font.GothamBlack;pctLbl.TextSize=12
    ...
    div1.BackgroundColor3=GRAY;div1.BackgroundTransparency=0.6;div1.BorderSizePixel=0;div1.ZIndex=25
...
-- ============================================================
-- MOBILE BUTTONS - Monochrome theme
-- ============================================================
...
local function destroyMobileButtons()
    ...
    for _,n in ipairs({"PhantomMobileButtons"}) do
        local old=game:GetService("CoreGui"):FindFirstChild(n);if old then old:Destroy() end
        local pgui=LP:FindFirstChild("PlayerGui");if pgui then local o=pgui:FindFirstChild(n);if o then o:Destroy() end end
    end
    mobBtnRefs={}
...
local function buildMobileButtons()
    ...
    mobGui.Name="PhantomMobileButtons";mobGui.ResetOnSpawn=false;mobGui.DisplayOrder=15;mobGui.IgnoreGuiInset=true
    ...
    mobGuiRef=mobGui
    local GRAY_ON      = Color3.fromRGB(180,180,180)
    local BTN_BG_OFF   = Color3.fromRGB(30,30,35)
    local PANEL_BG     = Color3.fromRGB(20,20,25)
    local BORDER_OFF   = Color3.fromRGB(80,80,85)
    local BORDER_ON    = Color3.fromRGB(180,180,180)
    local TEXT_OFF     = Color3.fromRGB(160,160,165)
    ...
    panelStroke.Color=Color3.fromRGB(60,60,65);panelStroke.Thickness=2;panelStroke.Transparency=0.3
    ...
    for i,def in ipairs(buttons) do
        ...
        local function setOn(v)
            ...
            if v then
                ...
                TweenService:Create(frame,TweenInfo.new(0.12,Enum.EasingStyle.Quad),{BackgroundColor3=GRAY_ON}):Play()
        ...
        local function flash()
            TweenService:Create(frame,TweenInfo.new(0.06),{BackgroundColor3=Color3.fromRGB(100,100,105)}):Play()
...
-- ============================================================
-- MAIN GUI — Phantom Hub style: dark gray/black/white theme
-- ============================================================
local function buildGui()
    clearPersistentConns();createStealBar()
    local BG         = Color3.fromRGB(12,12,15)
    local BG2        = Color3.fromRGB(20,20,25)
    local ROW_BG     = Color3.fromRGB(18,18,22)
    local ROW_BORDER = Color3.fromRGB(50,50,55)
    local WHITE      = Color3.fromRGB(255,255,255)
    local GRAY       = Color3.fromRGB(160,160,165)
    local INP        = Color3.fromRGB(10,10,13)
    local GRAY_ON    = Color3.fromRGB(180,180,180)
    local GRAY2      = Color3.fromRGB(80,80,85)
    local OFF        = Color3.fromRGB(30,30,35)
    local TAB_ACTIVE = Color3.fromRGB(200,200,200)
    local TAB_INACT  = Color3.fromRGB(100,100,105)
    local SECT_LBL   = Color3.fromRGB(140,140,145)
    for _,n in ipairs({"PhantomDuels"}) do
        local old=game:GetService("CoreGui"):FindFirstChild(n);if old then old:Destroy() end
        local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild(n);if o then o:Destroy() end end
    end
    local gui=Instance.new("ScreenGui");gui.Name="PhantomDuels";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
    ...
    local mfStroke=Instance.new("UIStroke",mainFrame);mfStroke.Color=GRAY;mfStroke.Thickness=1.5;mfStroke.Transparency=0.3
    task.spawn(function()
        local t=0
    ...
    _G._PhantomDuels_bgImage=bgImage
    ...
    hDivider.BackgroundColor3=GRAY;hDivider.BackgroundTransparency=0.6;hDivider.BorderSizePixel=0;hDivider.ZIndex=7
    dragHandle(header)
    local titleLbl=Instance.new("TextLabel",header)
    titleLbl.Size=UDim2.new(1,-80,1,0);titleLbl.Position=UDim2.new(0,12,0,0)
    titleLbl.BackgroundTransparency=1;titleLbl.Text="PHANTOM DUELS"
    ...
    discSmall.BackgroundTransparency=1;discSmall.Text="PHANTOM DUELS"
    discSmall.TextColor3=GRAY;discSmall.Font=Enum.Font.GothamBold
    ...
    closeBtn.BackgroundColor3=GRAY2;closeBtn.BorderSizePixel=0
    ...
    miniBtn.Text="PHANTOM DUELS";miniBtn.TextColor3=GRAY
    ...
    for _,name in ipairs(TABS) do
        ...
        sf.ScrollBarThickness=2;sf.ScrollBarImageColor3=GRAY2
    ...
    for _,name in ipairs(TABS) do
        ...
        underline.BackgroundColor3=GRAY_ON;underline.BorderSizePixel=0
    ...
    local function animPill(pill, dot, on)
        TweenService:Create(pill,TweenInfo.new(0.15,Enum.EasingStyle.Quad),{BackgroundColor3=on and GRAY_ON or OFF}):Play()
    ...
    local function mkBoxRow(parent, txt, default, cb)
        ...
        tb.Text=tostring(default);tb.TextColor3=GRAY_ON
    ...
    local function mkKBButton(row, kbEntry, cb)
        ...
        btn.Text=getLabel();btn.TextColor3=GRAY_ON
        ...
        btn.Activated:Connect(function()
            if li then li=false;_anyKeyListening=false;if lc then lc:Disconnect();lc=nil end;btn.Text=pv;btn.TextColor3=GRAY_ON;return end
            ...
            lc=UIS.InputBegan:Connect(function(inp)
                ...
                if inp.KeyCode==Enum.KeyCode.Escape then li=false;_anyKeyListening=false;if lc then lc:Disconnect();lc=nil end;btn.Text=pv;btn.TextColor3=GRAY_ON;return end
    ...
    local function mkActionRow(parent, txt, onActivate)
        ...
        btn.BackgroundColor3=GRAY2;btn.BorderSizePixel=0
        btn.Text="▶";btn.TextColor3=WHITE;btn.Font=Enum.Font.GothamBlack;btn.TextSize=11
        btn.AutoButtonColor=false;btn.ZIndex=9
        Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
        btn.Activated:Connect(function()
            TweenService:Create(btn,TweenInfo.new(0.06),{BackgroundColor3=Color3.fromRGB(60,60,65)}):Play()
            task.delay(0.12,function() TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=GRAY2}):Play() end)
    ...
    do
        ...
        manualBtn.BackgroundColor3=(infJumpMode=="manual") and GRAY_ON or Color3.fromRGB(40,40,45)
        ...
        holdBtn.BackgroundColor3=(infJumpMode=="hold") and GRAY_ON or Color3.fromRGB(40,40,45)
        ...
        manualBtn.Activated:Connect(function() infJumpMode="manual";manualBtn.BackgroundColor3=GRAY_ON;holdBtn.BackgroundColor3=Color3.fromRGB(40,40,45);stopHoldInfJump();saveConfig() end)
        holdBtn.Activated:Connect(function() infJumpMode="hold";holdBtn.BackgroundColor3=GRAY_ON;manualBtn.BackgroundColor3=Color3.fromRGB(40,40,45);if infJumpEnabled then startHoldInfJump() end;saveConfig() end)
    end
    -- STEAL TAB
    pg=tabPages["Steal"]
    ...
    do
        ...
        skyVal.TextColor3=GRAY_ON;skyVal.Font=Enum.Font.GothamBold;skyVal.TextSize=9
        ...
        cycleBtn.BackgroundColor3=GRAY2;cycleBtn.BorderSizePixel=0
        ...
        cycleBtn.Activated:Connect(function()
            ...
            PhantomApplyCustomSky(newTheme);saveConfig()
        end)
    end
    do
        ...
        fovBtn.Text=tostring(fovValue);fovBtn.TextColor3=GRAY_ON
    ...
    do
        ...
        resetBtn.BackgroundColor3=Color3.fromRGB(60,30,30);resetBtn.BorderSizePixel=0
    ...
    do
        ...
        sizeBox.Text=tostring(mobileButtonsSize);sizeBox.TextColor3=GRAY_ON
    ...
    do
        ...
        resetPosBtn.BackgroundColor3=GRAY2;resetPosBtn.BorderSizePixel=0
...
local function loadConfigKeys()
    if not(isfile and isfile("phantomduels.json")) then return end
    local ok,cfg=pcall(function() return HS:JSONDecode(readfile("phantomduels.json")) end)
    ...
    if cfg.chromeImageVisible~=nil then _G._PhantomDuels_bgImageVisible=cfg.chromeImageVisible==true end
    ...
    if cfg.skyTheme then currentSkyTheme=cfg.skyTheme;PhantomApplyCustomSky(currentSkyTheme) end
...
local function loadConfigState()
    ...
    task.spawn(function()
        ...
        if cfg.skyTheme then PhantomApplyCustomSky(cfg.skyTheme) end
...
if mobileButtonsEnabled then buildMobileButtons() end
loadConfigState()
PhantomApplyCustomSky(currentSkyTheme)
