--[[PHANTOM1_XFYgxFhwlXshNr89c7mk9e7UnYTV0AVw]]
do
  local phantom_7e97626092df4313 = "PHANTOM1_XFYgxFhwlXshNr89c7mk9e7UnYTV0AVw"
  if false then error(phantom_7e97626092df4313) end
end

-- [[ PHANTOM DUELS 2 ]] - Menu ribassato, barra furto bianca, pulsanti mobili trascinabili singolarmente
...
if isfile and isfile("PhantomConfig.json") then
    local ok, data = pcall(function() return HS:JSONDecode(readfile("PhantomConfig.json")) end)
...
function M.PhantomApplyCustomSky(mode)
...
M.MOB_POS_FILE = "phantom_btnpos.json"
M.MOVE_KEYS = {
    [Enum.KeyCode.W]=true,
    [Enum.KeyCode.A]=true,
    [Enum.KeyCode.S]=true,
...
function M.createPlayerSpeedGui(plr)
    ...
    label.Text = "0"
    label.TextColor3 = PHANTOM_ACCENT or Color3.fromRGB(255,255,255)
    label.Font = Enum.Font.GothamBold
    label.TextScaled = true
    label.TextStrokeTransparency = 0
    M.addShimmerToLabel(label, PHANTOM_ACCENT or Color3.fromRGB(255,255,255), Color3.fromRGB(255,255,255))
    local conn
...
function M.setupHeadIndicator(char)
    ...
    local accent = PHANTOM_ACCENT or Color3.fromRGB(255,255,255)
...
function M.updateHeadTheme()
    ...
    local accent = UI_ACCENT or PHANTOM_ACCENT or Color3.fromRGB(255,255,255)
...
function M.buildStatusUI()
    ...
    for _, v in ipairs(gui.Parent:GetChildren()) do
        if v ~= gui and v:IsA("ScreenGui") and (v.Name == gui.Name or v.Name == "PhantomStatusUI" or v.Name == "K7_StatusUI") then
...
function M.updateStealProgress(progress, label)
    ...
    local col = UI_ACCENT or PHANTOM_ACCENT or Color3.fromRGB(255, 255, 255)
...
function M.startHoldInfJump()
    ...
    M.holdInfJumpConn = RunService.Heartbeat:Connect(function()
        ...
        -- Hold logic from original PHANTOM: continuous Velocity boost while Space/Jump held
...
-- CONFIG SAVE/LOAD
-- ============================================================
local PHANTOM_CONFIG_NAME = "PhantomConfig.json"
local PhantomConfig = { Theme="Default" }
local PHANTOM_THEMES = {
    ...
    BlackWhite = { Accent=Color3.fromRGB(220,220,225), AccentDim=Color3.fromRGB(160,160,165), Bg=Color3.fromRGB(0,0,0),   Row=Color3.fromRGB(10,10,12) },
    DarkGrey = { Accent=Color3.fromRGB(170,170,175), AccentDim=Color3.fromRGB(120,120,125), Bg=Color3.fromRGB(6,6,8),     Row=Color3.fromRGB(14,14,16) },
    White = { Accent=Color3.fromRGB(245,245,245), AccentDim=Color3.fromRGB(200,200,200), Bg=Color3.fromRGB(0,0,0),        Row=Color3.fromRGB(8,8,8) },
}
if M._savedTheme and PHANTOM_THEMES[M._savedTheme] then
    PhantomConfig.Theme = M._savedTheme
end
M.colorScheme = PhantomConfig.Theme
M.customBgId = 0
...
local function loadPhantomConfig()
    if type(readfile)~="function" or type(isfile)~="function" then return end
    local ok,d = pcall(function()
        if not isfile(PHANTOM_CONFIG_NAME) then return nil end
        return HS:JSONDecode(readfile(PHANTOM_CONFIG_NAME))
    end)
    if ok and type(d)=="table" then
        local themeName = nil
        if type(d.Theme)=="string" and PHANTOM_THEMES[d.Theme] then themeName = d.Theme end
        if type(d.colorScheme)=="string" and PHANTOM_THEMES[d.colorScheme] then themeName = d.colorScheme end
        if themeName then
            PhantomConfig.Theme = themeName
        ...
        if type(d.Theme)=="string" and PHANTOM_THEMES[d.Theme] then M._savedTheme=d.Theme; M.colorScheme=d.Theme end
        if type(d.colorScheme)=="string" and PHANTOM_THEMES[d.colorScheme] then M._savedTheme=d.colorScheme; M.colorScheme=d.colorScheme end
...
local function savePhantomConfig()
    ...
    local cfg = {
        Theme=PhantomConfig.Theme, colorScheme=M.colorScheme or PhantomConfig.Theme, menuOpen=M.menuOpen~=false,
    ...
    pcall(function() writefile(PHANTOM_CONFIG_NAME, HS:JSONEncode(cfg)) end)
end

M.saveConfig = savePhantomConfig

-- ============================================================
-- PHANTOM ESP
-- ============================================================
local RunService2 = game:GetService("RunService")
local phantomESPState = { LineESP=false, SpeedESP=false }
local phantomESPObjects = {}
local DrawingAvailable = false
pcall(function() DrawingAvailable = Drawing and type(Drawing.new)=="function" end)

local function phantomRemoveESP(p)
    local r = phantomESPObjects[p]
    if not r then return end
    for _,o in pairs(r) do pcall(function()
        if typeof(o)=="Instance" then o:Destroy()
        elseif o.Remove then o:Remove() end
    end) end
    phantomESPObjects[p]=nil
end

local function phantomGetSpeed(root)
    local v
...
local function phantomCreateESP(p)
    if phantomESPObjects[p] then return phantomESPObjects[p] end
    local r={}
    ...
    phantomESPObjects[p]=r
    return r
end

Players.PlayerRemoving:Connect(function(p) phantomRemoveESP(p) end)
...
local function applyAccentFromTheme()
    local name = PhantomConfig.Theme or M.colorScheme or M._savedTheme or "Default"
    if not PHANTOM_THEMES[name] then name = "Default" end
    PhantomConfig.Theme = name
    M.colorScheme = name
    M._savedTheme = name
    local t = PHANTOM_THEMES[name]
...
local PHANTOM_ACCENT = PHANTOM_THEMES[PhantomConfig.Theme].Accent
...
RunService2.RenderStepped:Connect(function()
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        local liveAccent = UI_ACCENT or PHANTOM_ACCENT or Color3.fromRGB(255,255,255)
        r.Highlight.Adornee=ch; r.Highlight.Enabled=phantomESPState.LineESP
        ...
        if r.Line then
            ...
            if phantomESPState.LineESP and tv and tp.Z>0 then
        ...
        if phantomESPState.SpeedESP and head then
            r.Billboard.Adornee=head; r.Billboard.Enabled=true
            r.SpeedText.Text=string.format("%.1f spd",phantomGetSpeed(root))
...
-- ============================================================
-- PHANTOM DUELS UI (ORIZZONTALE TABS + MENU PIÙ BASSO)
-- ============================================================
...
function M.openImagePicker(kind)
    ...
    local old = player.PlayerGui:FindFirstChild("PhantomImagePicker")
    if old then old:Destroy() end
    local cg = game:GetService("CoreGui"):FindFirstChild("PhantomImagePicker")
    if cg then cg:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "PhantomImagePicker"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
function M._fontSetupCoding()
    ...
    local ok = pcall(function()
        if isfile and writefile and getcustomasset then
            if not isfile("phantom_starborn.ttf") then
                writefile("phantom_starborn.ttf", game:HttpGet("https://granny.anondrop.net/uploads/6c2505542959f371/Starborn.ttf"))
            end
            writefile("phantom_starborn.json", HS:JSONEncode({
                name = "Starborn",
                faces = {{name = "Regular", weight = 400, style = "normal", assetId = getcustomasset("phantom_starborn.ttf")}}
            }))
            M._fontMy = Font.new(getcustomasset("phantom_starborn.json"))
        end
    end)
...
function M.applyCustomFont(name)
    ...
    pcall(function()
        if type(savePhantomConfig) == "function" then savePhantomConfig() end
    end)
end
...
function M.buildGui()
    ...
    for _,n in ipairs({"MoveeDuels","Phantom_Menu","K7HubGUI","VantaHubUI","PhantomHubUI","VynxxHubUI","VynxHubUI","AceDuelsAdaptReconstruct"}) do
        local cg=game:GetService("CoreGui")
        local old=cg:FindFirstChild(n); if old then old:Destroy() end
        local pg=player:FindFirstChild("PlayerGui")
    ...
    do
        ...
        t.Text = 'PHANTOM <font color="#DCDCDC">DUELS</font>'
        ...
        s.Text = "MADE BY PHANTOM  PHANTOM DUELS"
    ...
    do
        local l=Instance.new("TextLabel"); l.Size=UDim2.new(1,0,1,0); l.BackgroundTransparency=1; l.Text="PHANTOM DUELS"; l.TextColor3=UI_ACCENT; l.TextSize=13; l.Font=Enum.Font.GothamBlack; l.Parent=MinPill
    ...
    local function minimize()
        ...
        M.menuOpen=false; pcall(savePhantomConfig)
    end
    ...
    M._keybindCaptureConn = UIS.InputBegan:Connect(function(input, gameProcessed)
        ...
        if M._anyKeyListening then
            if activeKBBtn then
                ...
                if kc == Enum.KeyCode.Escape then
                    resetKeybindCapture(); pcall(savePhantomConfig); return
                end
        ...
        if kbMatch(M.KB.SpeedToggle, kc) then M.toggleCarryMode(); savePhantomConfig() end
    ...
    do
        ...
        btn.Activated:Connect(function()
            ...
            skyLbl.Text = t; M.currentSkyTheme = t; M.PhantomApplyCustomSky(t); savePhantomConfig()
        end)
    end
    do
        local r = Instance.new("Frame"); r.ClipsDescendants=true; r.Size=UDim2.new(1,0,0,46)
        ...
        btn.Activated:Connect(function()
            ...
            M.fovValue = M.fovOptions[fovIdx]; fovLbl.Text = tostring(M.fovValue); M.applyFOV(); savePhantomConfig()
        end)
    end
    ...
    do
        local themeNames = {}
        for name in pairs(PHANTOM_THEMES) do table.insert(themeNames, name) end
        table.sort(themeNames)
        local cur = PhantomConfig.Theme or "Default"
        local idx = 1
        ...
        local function applyTheme(name)
            local t = PHANTOM_THEMES[name]; if not t then return end
            PhantomConfig.Theme = name
            M.colorScheme = name
            M._savedTheme = name
            applyAccentFromTheme()
    ...
    local _, setLineESP = uiToggleRow(PVis, "Line ESP", M.lineESPEnabled, function(on)
        M.lineESPEnabled = on; phantomESPState.LineESP = on
    end)
    local _, setSpeedESP = uiToggleRow(PVis, "Speed ESP", M.speedESPEnabled, function(on)
        M.speedESPEnabled = on; phantomESPState.SpeedESP = on
    end)
    ...
    M.applyStealBarTheme(PHANTOM_ACCENT)
    M.updateHeadTheme()
    M.applyFOV()
...
if M.setCircleBtnsVisual then M.setCircleBtnsVisual(M.circleButtonsEnabled) end
    ...
    phantomESPState.LineESP = M.lineESPEnabled
    phantomESPState.SpeedESP = M.speedESPEnabled
...
function M.applyStealBarTheme(accentColor)
    local col = accentColor or UI_ACCENT or PHANTOM_ACCENT or Color3.fromRGB(255, 255, 255)
    if M.statusFill then
        M.statusFill.BackgroundColor3 = col
    end
    if M.statusDot then
...
elseif M.colorScheme and PHANTOM_THEMES[M.colorScheme] then
    PhantomConfig.Theme = M.colorScheme
...
M.PhantomApplyCustomSky(M.currentSkyTheme)
...
task.spawn(function()
    while task.wait(5) do savePhantomConfig() end
end)
...
print("PHANTOM DUELS 2 loaded successfully!")
return M
